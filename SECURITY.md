...
- **Phoenix-13 hook.** One hook file (`hooks/coalwash-conductor.js`) branches on two registered events — SessionStart (the gauge) and UserPromptSubmit (the FULL-band per-turn repeat) — both fail-silent, zero-dependency, no network, **no child processes**, and silent except the sanctioned context-injection channel. A headless start is safe by construction — it only prints.
