...
Run the read-only gauge — the one-shot CLI from the engine `scripts/lib/`: `node "[LIB]/cli.mjs" gauge --json` (recover + discover + measure + verdict + break-even in one call; never hand-compose the lib calls) — plus the state file `~/.claude/projects/<slug>/coalwash/state.json` and the transaction dir `[project]/.claude/coalwash/`, and show:
