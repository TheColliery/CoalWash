...
- **Last run:** newest `snap-[timestamp]/` in the transaction dir (= the last apply's snapshot, with date) and whether the pre-gauge check above found a dangling `journal.json` (an interrupted run — the gauge call may have already resolved it as its one write exception; recommend `/coalwash` if it has not). No transaction dir → no run recorded.
