...
3. **Cadence.** To change how updates are handled, set `updateMode` (`ask` | `auto` | `remind` | `off`) and `updateCheckDays` — global `~/.claude/.coalwash.json`, overridable per project (`updateMode` is one of the safety keys, so a project may only quieten it, never escalate past a deliberate or unreadable global). `auto` lets this check run when due without re-asking; `off` silences it entirely.
