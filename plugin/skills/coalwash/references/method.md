...
**`handoffVerdict` thresholds:** knee 50,000 tok (the low edge of the ~50-60k single-worker degradation band — a REASONED placeholder like §9's rates; offer-early is the safe direction since the offer is declinable) · floor 4 files (CoalFace's own `autoFanoutFloor` factory default — its fan-out sense, not a new CoalWash knob). `fileCount <= 1` short-circuits to `single-worker` at ANY size — one file cannot be partitioned; advise demote-first (RE-TIER's valve shrinks the pile losslessly) instead of fan-out. `offer-coalface` = OFFER ONCE, only if CoalFace is installed — "this workload is fan-out grade → convene `/coalface`"; CF runs the swarm under its own discipline and wallet honesty (a $-and-speed bound — raw tokens run higher), and CW's fidelity gate stays the domain gate on every returned anchor-edit order. Decline, or CoalFace not installed → the single clone proceeds (multiple sittings via the §9b logbook). Never auto-convene, never spawn extra workers inside CoalWash — fan-out belongs to the sibling, extend-not-fork (the same seam as CT→CB).
...
2. **`keeps.json` appends do NOT fail closed.** `recordKeepAt` swallows a write failure — no throw, no abort — and nothing else in the engine surfaces it. But the signal it returns changed shape (grad11 F5, ascended from the class-b lane): the two early guard returns and the catch-path return all now share the swallowed-failure shape `{ ok: false, anchorDropped: false, anchorStored: false }`, against the success return's `{ ok: true, ... }`. **`{ ok: false }` is truthy** — the table tells you to check `.ok`, never a bare `if (!recordKeep(...))`, which can never fire against this return. A silently-lost append means the adjudicated *stand* is gone, and the item re-flags on the next run as though it had never been decided.
