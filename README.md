...
**Memory-BMI** = footprint / lean floor (floor-relative — real growth never false-fires), checked against **one hysteresis-gated ceiling**: 1.5×floor arms it, 1.2×floor disarms it — never a calendar. Armed = OBESE. *Footprint means what this project owns:* governance inherited from a parent directory above it — an umbrella repo's own `CLAUDE.md` and the files it imports — is measured and reported as its own figure but never counted into the band, because a project can neither wash nor externalize a file that isn't its to touch.

**FULL is the economic cut-point on that same ceiling**, not a separate line: armed + break-even proven (`cost(one run) < cost(carrying the fat)`, numbers always shown) fires FULL instantly — always a subset of OBESE, latching for the episode (a dip won't un-FULL it, only a clean reset does). A **capacity wall** forces FULL separately, no BMI needed — but the wall itself is **floor-relative once a lean floor is measured**: `fatMultiple × lean floor` (default `2.0`), clamped at the platform's true context capacity, so a legitimately-grown *muscle* store raises its own wall instead of false-firing against a fixed slice of capacity. Before any floor is measured, the wall falls back to the older fixed heuristic (`fullPercent` of context capacity — legacy pre-floor only). Capacity-nearing footprint is muscle, not fat: disarmed+wall → externalize/split, never wash-harder; armed+wall → real fat remains, wash first. Day one runs against a *provisional* floor (install footprint, BMI=1.00, tracks growth) until a gate-passed clean stamps the true floor — a day-one or too-small-to-trust-a-ratio store isn't over the ceiling, so only the wall forces its FULL. **Only Memory-BMI and the wall ever trigger a state change — never time or age.** A stat-only `Stop`-turn recheck (last gauge's known paths, no directory walk or content read) catches a within-session spike before the next `SessionStart`; full re-measure only re-runs on real drift, never unconditionally.
...
| `fatMultiple` | `2.0` | Growable capacity-wall multiplier: once a lean floor is measured, the wall is `fatMultiple × floor` (clamped at true context capacity) — raise it to consciously carry more muscle before the wall forces a run (min `1.6`, must stay above the OBESE ceiling or the wall would fire first) |
| `fullPercent` | `6` | Legacy pre-floor wall heuristic only — the fixed `%` of context capacity used before any lean floor has been measured; `fatMultiple` takes over once a floor is stamped |
