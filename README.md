...
A legacy read is not silent: `node scripts/lib/cli.mjs config-status` (also rendered by `/coalwash:stats`) names a resolved legacy path with the canonical replacement, in a one-line migration notice — deliberately a user-pulled report, not a SessionStart nag, since this room's own hermetic tests default every project to the root legacy shape and plenty of real installs do too. A `.coalwash.json` planted somewhere this walk will **never** read at all (a typo dir, a sibling room's own legacy shape under `.agents`/`.gemini`) is rarer and is reported ambiently instead, on the conductor's SessionStart line: `IGNORED: <path> is not a config path; canonical = .claude/coal/coalwash.json`. **There is no separate runtime deprecation-warning channel beyond those two** — Phoenix #13 (`CoalWorks/.claude/rules/hooks-safety.md`) confines a hook to its sanctioned emissions, and neither notice above is a general warning system; this note and the `### Deprecated` entry in [`CHANGELOG.md`](CHANGELOG.md) are the deprecation notice itself.
