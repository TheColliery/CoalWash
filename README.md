...
| `scanEverything` | `false` | Bypass every SCAN-scope cut for the run: the always-loaded read budget (262144 B) is lifted so every always-loaded entry is read and its certain fat measured instead of counting as muscle, and the 200-path cap on the Stop hook's re-stat baseline is not applied. Widens what is **seen**, never what is deleted — keeps, the delete gates, `localOnly` and every consent gate are untouched. Costs more than a normal gauge; discloses itself on SessionStart while on. Can't be turned ON by a project config |
