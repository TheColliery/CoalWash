...
**Docs:** [thecolliery.gitbook.io/thecolliery-docs/tools/coalwash](https://thecolliery.gitbook.io/thecolliery-docs/tools/coalwash) *(publishing soon)*
...
A keep's own `anchor`/`anchorFile`-based structural re-check — verifying the anchor text still sits where it was recorded before honoring an old keep — is **lab-only**, declared 2026-08-04, and not wired into any shipped write path: nothing in the engine currently produces an anchor-bearing keep record, so every live keep is the plain form above. The file-level `pinned: true` gate (Apply, above) is a separate, wired, fully-exercised mechanism, unaffected.
...
| **Write guard (0p)** | Other hands edit the store daily too (a subagent, a stray edit, another tool), so the fidelity gate gets an **advisory** twin on the write path: an **airbag** snapshots a file's first write each session made through the file-edit tools (Edit/Write/MultiEdit) — the undo net for gitignored `MEMORY.md`/`CLAUDE.md` on that channel only; a Bash-mediated move (`mv`, `sed`, a script) takes no snapshot at all. A **seatbelt** flags a dropped link/number/quote with one FYI line pointing at the snapshot. **Never blocks** — a deliberate delete is legitimate, so it informs, not gates; a false alarm costs one ignorable line. In a project whose `.claude` or `.claude/coalwash` is itself a link (a symlink or junction) no snapshot is taken and nothing is swept through it, and `apply` refuses loudly, naming the link; a governance or memory file over 4 MiB is no longer snapshotted or content-measured (its `@import` closure is flagged instead). Reaches inside subagents (tool hooks do), silent on clean edits; recovery copies the real bytes, never an AI re-typing what it thinks was there. Off: `writeGuard: "snapshot-only"` or `"off"` |
...
| **Activation ladder** | Capability-keyed: lifecycle hooks → the gauge runs automatically (Claude Code today); no hooks → a best-effort agent-driven offer is designed, **not shipped on a file-copy install** (nothing installs the always-loaded instruction it needs); always → manual `/coalwash`. Band crossings resolve on the `Stop` hook (the same blocking channel `rot-canary` uses), so whatever surfaces is enforced, not suggested — edge-triggered, never a repeating nag. A hookless platform sits at manual today |
...
Every tool in the series supports two config levels — a global `~/.claude/.coalwash.json` and a per-project override — so a globally-installed skill can be tuned or **shut off per project** (`coalwashMode: "off"` is the off-switch) — a skill you don't need in a given project stops loading (and burning tokens) there. **Project wins, except for a handful of safety keys** (`coalwashMode`, `updateMode`, `writeGuard`, `localOnly`, `scanEverything`, `estate.deleteCold`) — a cloned project's config may only make those *quieter*, never weaken a deliberate global choice or an unreadable one; every other key is plain project-wins — except `estate.archiveDir`, which is not a consent key but a REACH key: it is read from the **global** config only, and a value set in a project config is **ignored** (a cloned repo has no business choosing where your transcripts are archived; with no global value the archive lands under `~/.claude/coal/coalwash/estate-archive`). If you previously archived with a project-level `archiveDir`, `estate-search` and `estate-restore` now look at the global or default directory: the files in the old one stay on disk, and moving the value with `node scripts/configure.mjs --global --estate.archiveDir <path>` points the tools back at them. Both commands also say so on stderr, on every run, whenever a project config carries such a value that was ignored: the line says why (a cloned repo ships a project config and must not choose where your transcripts are archived), what the project config asked for and the directory the run read, and, if you set that path yourself, where to set it or which archives to move (stdout and the exit code are unchanged; a project value that only restates your global one, or a relative one, draws nothing).

**Where the per-project override lives — the read order (identical across the series, one flock):** (1) the canonical path, `<project>/.claude/coal/coalwash.json` — the dir of the agent actually executing (Claude Code: `.claude`); (2) other known agent dirs, fixed order `.claude` → `.agents` → `.gemini` (first found wins); (3) **DEPRECATED:** `<project>/.claude/.coalwash.json` — the nested legacy shape shared with CoalTipple's and CoalBoard's own convention; (4) **DEPRECATED:** `<project>/.coalwash.json` at the project root — the original (pre-2026-08-08) shape. Both legacy paths are still read, nested-before-root, canonical always winning over either. **Deprecated at this release (MINOR), removable at CoalWash's next MAJOR** — this flock's own SemVer boundary, never a months-count; the migration owner is this room. Move a legacy config to the canonical path above when convenient — reading it keeps working exactly as it does today until removal, and nothing is auto-moved for you.

A legacy read is not silent: `node scripts/lib/cli.mjs config-status` (also rendered by `/coalwash:stats`) names a resolved legacy path with the canonical replacement, in a one-line migration notice — deliberately a user-pulled report, not a SessionStart nag, since this room's own hermetic tests default every project to the root legacy shape and plenty of real installs do too. A `.coalwash.json` planted under `.agents`/`.gemini` bare — the same bare-dotfile habit, one directory over, but a shape this walk carries no nested-legacy candidate for — is rarer and is reported ambiently instead, on the conductor's SessionStart line: `IGNORED: <path> is not a config path; canonical = .claude/coal/coalwash.json`. **This report checks exactly those two known directory names — it does not walk the tree, so a genuinely misspelled directory is invisible to it, not merely rare.** A config that EXISTS where the walk reads but cannot be used is reported on that same SessionStart line, once: `UNREADABLE: <path> exists but is not a readable config (<reason>); it was skipped — canonical = <canonical>`, where `<reason>` is one of `malformed JSON`, `a directory`, `not a JSON object` or `unreadable`. The file is skipped and the walk's selection is unchanged; the project tier's canonical is `.claude/coal/coalwash.json`, and the global tier names its own path there, since a global config has nowhere to move to. An unreadable GLOBAL config still prints its line even though it makes `coalwashMode` fail safe to `off`; a readable `off` stays fully silent, and so does an unreadable project config beneath it. A project config over 1 MiB, a FIFO or device, or a link that dangles or leaves the project reads as `unreadable`. `config-status` does not carry this line. **There is no separate runtime deprecation-warning channel beyond those notices** — Phoenix #13 (`CoalWorks/.claude/rules/hooks-safety.md`) confines a hook to its sanctioned emissions, and none of the notices above is a general warning system; this note and the `### Deprecated` entry in [`CHANGELOG.md`](CHANGELOG.md) are the deprecation notice itself.

Whichever candidate is found is where an edit takes effect; nothing is auto-moved on read. Write with `node scripts/configure.mjs <flags>` (the project config) or `--global <flags>` (the global layer) — every flag is the key spelled exactly as the schema spells it, dotted for a nested one (`--estate.digCrush.pileTok`, `--retier.armPct`). A write lands back wherever the config was already found; a project with no config yet gets the first agent dir it **already has** on disk (`.claude` → `.agents` → `.gemini`), never a bare `.claude` planted ahead of an `.agents`/`.gemini` dir that already exists. A config sitting at either LEGACY path is written back **there**, not moved — unlike some siblings, this room performs no migrate-and-delete. Invalid input changes nothing: the whole run is rejected and the file is left byte-identical — including a config whose body is `null`, `0`, `false` or an empty string, which is refused as not a JSON object instead of being accepted as an empty config. Writing `estate.archiveDir` to a project config still writes the value but warns that the key is read from the global config only, and points at `--global --estate.archiveDir`. A project-config write that would land through a link, or outside the project, is refused: `[refused] <path>: <why> -- nothing was written. ...`, exit 1 (`--global` is written as before).

**The global layer, `~/.claude/.coalwash.json`, is unchanged and NOT deprecated** — the two paths above are project-level only; do not read the deprecation as touching the global config, which stays at its one, always-current shape. The main keys:
...
| `scanEverything` | `false` | Bypass every SCAN-scope cut for the run: the always-loaded read budget (262144 B) is lifted so no always-loaded entry is skipped for budget and its certain fat is measured instead of counting as muscle by default (an entry whose read *fails* still contributes nothing — that path is unchanged), and the 200-path cap on the Stop hook's re-stat baseline is not applied. Widens what is **seen**, never what is deleted — keeps, the delete gates, `localOnly` and every consent gate are untouched. Costs more than a normal gauge; discloses itself on SessionStart while on. Can't be turned ON by a project config |
...
Full key reference: every key + default lives in [`scripts/lib/config-schema.mjs`](scripts/lib/config-schema.mjs) and the commented template [`platform-configs/.coalwash.json`](platform-configs/.coalwash.json) — or run `node scripts/configure.mjs --help`, whose every flag, bound and default is generated from that same schema table, so a key added there is settable and documented from one source.
...
**A repo checkout adds one more writer, outside the installed skill: `scripts/configure.mjs`** (the config CLI above) is not part of the plugin you install — `plugin/` carries only `scripts/lib`, so an installed-only user never receives it. Run from a checkout, it writes the project (or, with `--global`, the global) `.coalwash.json` and nothing else: no memory file, no transcript, no snapshot. A rejected run writes nothing at all.
