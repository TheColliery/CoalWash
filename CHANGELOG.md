...
### Security
- **[HIGH] The Claude config dir could become a trusted apply root.** `~/.claude/CLAUDE.md` — the user's own global instruction file, present on most installs — matched the newly added `CLAUDE.md` project-root marker, so any session whose working directory sat under `~/.claude` resolved its project root to `~/.claude` itself. That directory then entered `applyPlan`'s trusted roots, and the existing home-swallow guard did not fire because `~/.claude` sits *below* home: a forged `PLAN.json` declaring `roots: [~/.claude]` could write a `SessionStart` command hook into `settings.json`, i.e. code execution on the next session. The base directory is now never a project root — a working directory beneath it with no other marker falls back to the fail-closed behaviour it had before the marker existed, while real projects continue to resolve on `.git` / `.coalwash.json` / their own `CLAUDE.md`. The exclusion is derived from the same base path the rest of the engine uses, so it follows `CLAUDE_CONFIG_DIR`. Introduced in this same unreleased cycle by the state-scatter fix below and caught before release; no shipped version was affected.
