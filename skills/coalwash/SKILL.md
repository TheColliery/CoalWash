...
name: coalwash
description: >-
  Memory washer/defragmenter for agent class-B memory (memory + governance files) — cleans the FAT, never the MEAT. Fidelity-first: a free mechanical Quick pass + a CODE-enforced fidelity gate (zero fact-loss proven by diff, not hoped); the paid semantic Full pass is a separate consent; every DELETE/MERGE is human-gated in code (pinned files untouchable). Per-project, per-session-exclusive (a lock defers when another CoalWash run holds the store). Session-start gauge, 4 bands: LEAN silent · PLUMP ask · OBESE strong-ask · FULL force-runs ONLY on a deterministic break-even proof, numbers shown (deletes still human-gated). NO calendar cadence — never loop it (past fat-exhaustion, cleaning risks the meat). localOnly = Quick-only, no sub receives memory content. Honest frame: slows memory-overhead growth — does NOT eliminate memory cost. Triggers: "/coalwash", "clean memory", "defrag memory", "memory bloat", a [CoalWash] band nudge. Cross-agent design (Claude Code validated). Zero-dependency, offline, no API keys.
---

# CoalWash — the memory washer

> **Honest frame:** coal washing cleans raw coal without damaging the coal. CoalWash cuts class-B bloat (memory + governance files the platform loads each session) under a **zero-fact-loss** constraint — fidelity proven by CODE (a mechanical diff gate), deletes gated by the HUMAN, the apply transactional (snapshot + WAL + rollback: worst crash outcome = "the run did not happen", never "memory corrupted"). It **slows how fast memory-overhead grows** — it does NOT eliminate memory cost, reach zero, or stop legitimate growth (the meat-floor rises; that is health, not fat).
...
- **`localOnly: true` = Quick tier only, zero transmission.** No spawned sub ever receives memory content; the semantic Full tier is disabled. Restrict content-touching work to the always-loaded set (already in your context via the platform); recall-store files get code measurement + flags only.
