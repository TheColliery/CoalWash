...
> **Class-A estate (ULTRA + RE-TIER's byte lane) — a DIFFERENT, STRONGER contract:** transcripts/tool-results are vendor-owned + machine-parsed (fail the 4 wash tests) → **NEVER semantic-edited**; the guarantee is **byte-identity** (copy-verify-then-delete, `estate-restore` round-trips) — not the structured-token gate above. The one seam where the class-B gate re-enters = RE-TIER's hot-index rewrite (fidelity gate + #54 anchor bind exactly there). Detail → `references/method.md` §10/§11.
...
1. **Entry (neutral):** `neutralScan` (never calls `bandVerdict`, so no band/BMI leaks before the choice). Neutral header + exactly four descriptive choices: **"Fat only"** (broom + you, no outsider) vs **"Fat + reorganize muscle"** (adds the zero-context outsider — the same Full-tier contract as pipeline step 2) vs **"ULTRA — + estate"** (old transcripts/tool-results: compress + index, reclaim disk) vs **"RE-TIER — merge class-B → redistribute by the envelope (overflow demotes, nothing deleted)"**.
...
**RE-TIER (the fourth choice — separation of powers; envelope + table + commands in method §11):** the ENVELOPE (config `retier`, a ± band around the cross-AI target) decides TIER PLACEMENT ONLY; the per-type TREATMENT TABLE is CODE (anything stronger than a type's cell is refused). Envelope pressure resolves ONLY by lossless DEMOTION down the ladder (index line → topic file → estate archive, byte-identical, reachable back) — pressure NEVER escalates a treatment (ข้าม never becomes ย่อ "because it wouldn't fit"), and RE-TIER never summarizes or deletes content. Its bill replaces step 3: `cli.mjs retier-scan` AFTER the choice (the neutral entry stays numberless); run on เริ่ม via `cli.mjs retier-run` (print its report verbatim; it REFUSES below the arm line — "dead zone, no action"). Wizard-ONLY — never ambient, never band/BMI-triggered (same law as ULTRA).
