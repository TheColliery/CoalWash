...
2. **`keeps.json` appends do NOT fail closed.** `recordKeepAt` swallows a write failure — no throw, no abort — and nothing else in the engine surfaces it. But the signal it returns changed shape (grad11 F5, ascended from the class-b lane): a swallowed failure now returns `{ ok: false, anchorDropped: false, anchorStored: false }`, verified at `keeps.mjs:99`, `:102`, `:281`, against the success shape `{ ok: true, ... }` at `:279`. **`{ ok: false }` is truthy** — the table tells you to check `.ok`, never a bare `if (!recordKeep(...))`, which can never fire against this return. A silently-lost append means the adjudicated *stand* is gone, and the item re-flags on the next run as though it had never been decided.
